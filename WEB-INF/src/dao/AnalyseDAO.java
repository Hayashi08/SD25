...
public class AnalyseDAO extends DAO {
    ...
    // ジャンル別に全件検索
    public ArrayList<String> menu_searchall(String genre)throws Exception {

        // Beanのリスト
    	ArrayList<String> menulist = new ArrayList<String>();
        
        // SQL文
        String sql = "select menu_name from menu where menu_genre = ? order by menu_name asc";
        // STATEMENTの生成
    ...
    public ArrayList<String> analyse_menu(
        ...
        if(yaxis.equals("sales")){
        	sql +=		"SUM(menu_price * menu_qty)";;
        }
        if(yaxis.equals("noo")){
        	sql +=		"SUM(menu_qty)";
        }
        sql +=			" FROM sale_detail , menu , situation " +
        				"where " +
        				//saleテーブルとuserテーブルを結合
        				"sale_detail.situation_id = situation.situation_id and " +
        				//saleテーブルとsituationテーブルを結合
        				"sale_detail.menu_id = menu.menu_id and ";
        ...
        if(checked_menu != null){
        	sql +=		"menu_name in (";
        	for(int k = 0 ; k < checked_menu.length ; k++){
        		sql +=	"?";
        		if(k != checked_menu.length-1){
        			sql +=	",";
        		}
        	}
        	sql +=		") and ";
      ...
      //曜日の絞り込み
        ...
        if(!dotw_check){//曜日の絞り込み
        	sql +=		"WEEKDAY(situation_date) IN (" +
        				dotw_num +
        				") and ";
        }
        				//日付の絞り込み
        if(!xaxis.equals("week")){
        	sql +=		"situation_date like ?";
        }
        else{
...
//        }
        for(int j = 0 ; j < i ; j++){
            ...
            if(checked_menu != null){
            	for(int k = 0 ; k < checked_menu.length ; k++){
                	statement.setString(wildcard_cnt,checked_menu[k]);
                	wildcard_cnt++;
            	}
            }
